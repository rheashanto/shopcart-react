...
const FEATURES_MAP = {
  "men's clothing":   ['Premium fabric blend','Machine washable','Slim fit design','Reinforced stitching','Available in multiple sizes'],
  "women's clothing": ['Premium fabric blend','Water-resistant coating','Adjustable fit','Wrinkle-resistant material','Eco-friendly production'],
  'electronics':      ['High performance chipset','1-year manufacturer warranty','Energy efficient design','Compatible with major platforms','Compact and lightweight'],
  'jewelery':         ['925 sterling silver / gold plated','Hypoallergenic materials','Tarnish-resistant finish','Gift box included','Certified authentic'],
}
...
export default function ProductDetail() {
  ...
  const handleAddToCart = () => {
    if (!product) return
    // Fix 2: pass selected color & size as part of the cart item
    // Use a composite key so different size/color combos are separate cart entries
    const variantSuffix = [selectedColor, selectedSize].filter(Boolean).join('-')
    const cartId = variantSuffix ? `${product.id}-${variantSuffix}` : product.id
    addItem({
      id: cartId,
      title: product.title + (variantSuffix ? ` (${[selectedColor, selectedSize].filter(Boolean).join(', ')})` : ''),
      price: product.price,
      image: product.image,
          ...
          <div className="pd__info-col">
            ...
            <div className="pd__option-group">
              <p className="pd__option-label">
                Color:{selectedColor && <span className="pd__option-selected"> {selectedColor}</span>}
              </p>
              <div className="pd__option-btns">
                {colors.map(c => (
                  <button key={c} className={`pd__option-btn${selectedColor === c ? ' active' : ''}`}
            ...
            {isClothing && (
              <div className="pd__option-group">
                <p className="pd__option-label">
                  Size:{selectedSize && <span className="pd__option-selected"> {selectedSize}</span>}
                </p>
                <div className="pd__option-btns">
                  {SIZES.map(s => (
                    <button key={s} className={`pd__option-btn pd__size-btn${selectedSize === s ? ' active' : ''}`}
            ...
            <div className="pd__actions">
              <button className={`pd__add-btn${added ? ' added' : ''}`} onClick={handleAddToCart}>
              ...
              <button className={`pd__wishlist-btn${wishlist ? ' active' : ''}`}
                onClick={() => setWishlist(v => !v)} aria-label="Add to wishlist">
              ...
              <ul className="pd__features-list">
                {features.map(f => (
                  <li key={f} className="pd__feature-item">
                    <span className="pd__feature-check">✓</span>{f}
                  </li>
                ))}
              </ul>
            </div>
